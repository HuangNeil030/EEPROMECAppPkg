...
typedef struct {
  ...
  // EC RAM addresses
  UINT16 CmdBuffer;
  UINT16 DataOfCmdBuffer;
  UINT16 CmdWriteDataBuffer;
  UINT16 CmdCntl;
  UINT16 CmdReturnDataBuffer;
...
{
  while (TimeoutUs > 0) {
    UINT8 v = IndexIoRead8(mEc.CmdCntl);
    if (WantSet) {
      if ((v & Mask) == Mask) return EFI_SUCCESS;
    } else {
      if ((v & Mask) == 0) return EFI_SUCCESS;
    }
    gBS->Stall(50);
    TimeoutUs = (TimeoutUs > 50) ? (TimeoutUs - 50) : 0;
  }
  ...
  )
{
  ...
  Status = IndexWaitCmdCntlBit(CMD_CNTL_PROCESSING, FALSE, 200000);
  if (EFI_ERROR(Status)) return Status;

  IndexIoWrite8(mEc.CmdCntl, CMD_CNTL_PROCESSING);
  IndexIoWrite8(mEc.CmdCntl, (UINT8)(CMD_CNTL_PROCESSING | CMD_CNTL_START));

  Status = IndexWaitCmdCntlBit(CMD_CNTL_START, FALSE, 500000);
  if (EFI_ERROR(Status)) return Status;
  ...
  )
{
  ...
  IndexIoWrite8(mEc.DataOfCmdBuffer, EC_CMD_EEPROM_BANK_NUM);
  IndexIoWrite8(mEc.BankBuf, Bank);
  return IndexRunCmd();
}
  ...
  )
{
  ...
  IndexIoWrite8(mEc.DataOfCmdBuffer, EC_CMD_EEPROM_READ);
  IndexIoWrite8(mEc.ReadAddrBuf, Addr);

  {
    EFI_STATUS Status = IndexRunCmd();
    if (EFI_ERROR(Status)) return Status;
  }
  ...
  )
{
  ...
  IndexIoWrite8(mEc.DataOfCmdBuffer, EC_CMD_EEPROM_WRITE);
  IndexIoWrite8(mEc.WriteAddrBuf, Addr);
  IndexIoWrite8(mEc.WriteDataBuf, Data);
  return IndexRunCmd();
}
  ...
  )
{
  ...
  while (TRUE) {
    ...
    // I: cycle access backend (PortIO / IndexIO-ENE / IndexIO-Nuvoton / IndexIO-ITE)
    ...
    // ENTER: write by mode (BYTE/WORD/DWORD)
